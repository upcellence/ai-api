...
export default defineEventHandler(async ({ req }) => {
    const { prompt, title, specs, description_html, selected_voice, keywords } = (await req.json());
    ...
    if (req.method.toUpperCase() === 'POST') {
        ...
        return productSchema.parse(JSON.parse(response.text ?? JSON.stringify({ ...response.candidates?.[0] })));
    }
    else {
        return { message: 'Method not allowed', status: 405 };
    }
});
//# sourceMappingURL=humanize-product.mjs.map